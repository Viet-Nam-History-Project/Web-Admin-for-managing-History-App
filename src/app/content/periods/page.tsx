...
export default async function PeriodsPage() {
  ...
  return (
    <AdminShell>
      <PageHeader eyebrow="Nội dung" title="Quản lý thời kỳ lịch sử" description="Thời kỳ là trung tâm quản lý. Giai đoạn và sự kiện được mở trong đúng ngữ cảnh thời kỳ cha."
        actions={<><ExportPeriodsButton data={periods.map((period) => ({ id: period.id, slug: period.slug ?? period.id, title: period.title, summary: period.summary ?? '', description: period.description ?? '', coverMediaRef: period.coverMediaRef ?? '', startDate: formatHistoricalRange(period.startDate, period.startDate).split(' - ')[0], endDate: formatHistoricalRange(period.endDate, period.endDate).split(' - ')[0], sortOrder: period.sortOrder ?? 0, status: period.status ?? 'draft', stageCount: period.stageCount ?? 0, eventCount: period.eventCount ?? 0 }))} /><Link href="/content/trash"><Button variant="outline"><Trash2 className="h-4 w-4" /> Thùng rác</Button></Link><Link href="/content/periods/new"><Button><Plus className="h-4 w-4" /> Tạo thời kỳ</Button></Link></>} />
        ...
        <DataTable className="min-w-[1320px]">
          ...
          <tbody>{periods.map((period) => {
            ...
            return <TableRow key={period.id}>
              <TableCell><div className="h-12 w-20 overflow-hidden rounded-md bg-stone-200">{period.coverMediaRef ? <img src={period.coverMediaRef} alt="" className="h-full w-full object-cover" /> : null}</div></TableCell>

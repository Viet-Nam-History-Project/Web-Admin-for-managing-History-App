...
export default async function StageDetailPage({ params, searchParams }: { params: Promise<{ periodSlug: string; stageSlug: string }>; searchParams: Promise<{ tab?: string }> }) {
  ...
  const tabs = [['overview', 'Tổng quan'], ['events', `Sự kiện (${events.length})`], ['learning', 'Nội dung học'], ['persons', 'Nhân vật'], ['quiz', 'Quiz'], ['quality', 'Chất lượng'], ['graph', 'Kho tri thức'], ['audit', 'Nhật ký hoạt động']];
  return <AdminShell>
    <div className="mb-3"><BackButton fallbackHref={`/content/periods/${periodSlug}?tab=stages`} label="Quay lại thời kỳ" /></div>
    ...
    <PageHeader eyebrow="Giai đoạn" title={stage.title} description={formatHistoricalRange(stage.startDate, stage.endDate)} actions={<><Link href={`/content/periods/${periodSlug}/stages/${stageSlug}/edit`}><Button variant="outline">Sửa</Button></Link><EntityActionButton url={`/api/admin/periods/${periodSlug}/stages/${stageSlug}?action=${stage.status === 'published' ? 'unpublish' : 'publish'}`} label={stage.status === 'published' ? 'Gỡ xuất bản' : 'Xuất bản'} /><EntityActionButton method="DELETE" url={`/api/admin/periods/${periodSlug}/stages/${stageSlug}`} variant="danger" label="Xóa" confirmMessage="Chuyển giai đoạn này vào thùng rác? Các sự kiện liên quan vẫn được giữ để có thể khôi phục." /></>} />
    ...
    {active === 'overview' ? <div className="grid gap-5 lg:grid-cols-2"><Card className="overflow-hidden p-0"><div className="aspect-[16/10] bg-stone-200"><CoverImage source={stage.coverMediaRef} alt={stage.title} /></div></Card><Card><CardTitle>Nội dung giai đoạn</CardTitle><Badge className="mt-3" tone={stage.status === 'published' ? 'green' : 'gold'}>{statusLabel(stage.status)}</Badge><p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-stone-700">{stage.overview || stage.description || 'Chưa có nội dung tổng quan.'}</p><p className="mt-4 text-sm text-stone-500">Tác động: {stage.impactOnPresent || 'Chưa cập nhật'}</p></Card></div> : null}
    {active === 'events' ? <section><div className="mb-4 flex justify-end"><Link href={`/content/periods/${periodSlug}/stages/${stageSlug}/events/new`}><Button><Plus className="h-4 w-4" /> Tạo sự kiện</Button></Link></div>{events.length ? <DataTable><TableHead><TableRow><TableHeaderCell>Sự kiện</TableHeaderCell><TableHeaderCell>Niên đại</TableHeaderCell><TableHeaderCell>Trạng thái</TableHeaderCell><TableHeaderCell>Ảnh</TableHeaderCell><TableHeaderCell>Video</TableHeaderCell><TableHeaderCell></TableHeaderCell></TableRow></TableHead><tbody>{events.map((event) => <TableRow key={event.id}><TableCell><Link className="font-black text-charcoal" href={`/content/periods/${periodSlug}/stages/${stageSlug}/events/${event.id}`}>{event.title}</Link></TableCell><TableCell>{formatHistoricalRange(event.startDate, event.endDate)}</TableCell><TableCell><Badge tone={event.status === 'published' ? 'green' : 'gold'}>{statusLabel(event.status)}</Badge></TableCell><TableCell>{event.coverMediaRef ? 'Có' : 'Thiếu'}</TableCell><TableCell>{event.videos?.length || event.youtubeId ? 'Có' : 'Thiếu'}</TableCell><TableCell><div className="flex gap-2"><Link href={`/content/periods/${periodSlug}/stages/${stageSlug}/events/${event.id}`}><Button variant="ghost">Mở</Button></Link><Link href={`/content/periods/${periodSlug}/stages/${stageSlug}/events/${event.id}/edit`}><Button variant="outline">Sửa</Button></Link></div></TableCell></TableRow>)}</tbody></DataTable> : <EmptyState title="Chưa có sự kiện" description="Tạo sự kiện đầu tiên trong giai đoạn này." />}</section> : null}
    ...
    {active === 'quality' ? <Card><CardTitle>Kiểm tra chất lượng</CardTitle><div className="mt-4 grid gap-3 sm:grid-cols-4"><Quality label="Ảnh" ok={Boolean(stage.coverMediaRef)} /><Quality label="Tổng quan" ok={Boolean(stage.overview)} /><Quality label="Nội dung" ok={Boolean(stage.details?.length || stage.result?.length)} /><Quality label="Sự kiện" ok={events.length > 0} /></div></Card> : null}
    {['persons', 'quiz', 'graph', 'audit'].includes(active) ? <EmptyState title="Chưa có dữ liệu liên kết" description="Nội dung sẽ xuất hiện sau khi được liên kết với giai đoạn này." /> : null}
  </AdminShell>;
}
...
function statusLabel(status?: string) { return status === 'published' ? 'Đã xuất bản' : status === 'archived' ? 'Lưu trữ' : 'Bản nháp'; }

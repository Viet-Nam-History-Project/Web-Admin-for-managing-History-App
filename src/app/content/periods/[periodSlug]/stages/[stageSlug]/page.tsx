...
export default async function StageDetailPage({ params, searchParams }: { params: Promise<{ periodSlug: string; stageSlug: string }>; searchParams: Promise<{ tab?: string }> }) {
  ...
  return <AdminShell>
    ...
    {active === 'overview' ? <div className="grid gap-5 lg:grid-cols-2"><Card className="overflow-hidden p-0"><div className="aspect-[16/10] bg-stone-200"><CoverImage source={stage.coverMediaRef} alt={stage.title} /></div></Card><Card><CardTitle>Nội dung giai đoạn</CardTitle><Badge className="mt-3" tone={stage.status === 'published' ? 'green' : 'gold'}>{stage.status}</Badge><p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-stone-700">{stage.overview || stage.description || 'Chưa có overview.'}</p><p className="mt-4 text-sm text-stone-500">Tác động: {stage.impactOnPresent || 'Chưa cập nhật'}</p></Card></div> : null}

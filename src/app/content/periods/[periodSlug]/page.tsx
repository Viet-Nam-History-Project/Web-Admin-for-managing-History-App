...
export default async function PeriodDetailAdminPage({ params, searchParams }: { params: Promise<{ periodSlug: string }>; searchParams: Promise<{ tab?: string }> }) {
  ...
  return <AdminShell>
    <div className="mb-3 text-sm text-stone-500"><Link href="/content/periods" className="font-bold text-bronze">Thời kỳ</Link> / {period.title}</div>
    <PageHeader eyebrow="Trung tâm quản lý thời kỳ" title={period.title} description={`${formatHistoricalRange(period.startDate, period.endDate)} · ${stages.length} giai đoạn · ${events.length} sự kiện`}
      actions={<><Link href={`/content/periods/${periodSlug}/edit`}><Button variant="outline"><Edit3 className="h-4 w-4" /> Sửa</Button></Link><EntityActionButton url={`/api/admin/periods/${periodSlug}?action=${period.status === 'published' ? 'unpublish' : 'publish'}`} label={period.status === 'published' ? 'Gỡ xuất bản' : 'Xuất bản'} /><EntityActionButton url={`/api/admin/periods/${periodSlug}?action=sync`} label="Sync graph" /><EntityActionButton method="DELETE" url={`/api/admin/periods/${periodSlug}`} variant="danger" label="Xóa mềm" confirmMessage="Xóa mềm thời kỳ này? Toàn bộ dữ liệu con vẫn được giữ để phục hồi." /></>} />

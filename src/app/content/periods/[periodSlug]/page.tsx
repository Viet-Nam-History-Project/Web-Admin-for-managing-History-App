...
export default async function PeriodDetailAdminPage({ params, searchParams }: { params: Promise<{ periodSlug: string }>; searchParams: Promise<{ tab?: string }> }) {
  ...
  return <AdminShell>
    ...
    {['persons', 'quizzes'].includes(activeTab) ? <EmptyState title={activeTab === 'persons' ? 'Nhân vật liên quan' : 'Quiz liên quan'} description="Nội dung sẽ xuất hiện sau khi được liên kết với thời kỳ này." /> : null}
  </AdminShell>;
}
...
function statusLabel(status?: string) { return status === 'published' ? 'Đã xuất bản' : status === 'archived' ? 'Lưu trữ' : status === 'deleted' ? 'Đã xóa' : 'Bản nháp'; }
function actionLabel(action?: string) { return ({ create: 'Tạo mới', update: 'Cập nhật', publish: 'Xuất bản', unpublish: 'Gỡ xuất bản', soft_delete: 'Chuyển vào thùng rác', restore: 'Khôi phục', delete: 'Xóa' } as Record<string, string>)[action ?? ''] ?? 'Cập nhật'; }
function AuditTable({ items }: { items: Awaited<ReturnType<typeof auditAdminService.list>>['items'] }) { return items.length ? <DataTable><TableHead><TableRow><TableHeaderCell>Thời gian</TableHeaderCell><TableHeaderCell>Người thao tác</TableHeaderCell><TableHeaderCell>Hành động</TableHeaderCell><TableHeaderCell>Đối tượng</TableHeaderCell></TableRow></TableHead><tbody>{items.map((item) => <TableRow key={item.id}><TableCell>{formatAdminDateTime(item.createdAt)}</TableCell><TableCell>{item.actorEmail ?? 'Hệ thống'}</TableCell><TableCell><Badge tone="gold">{actionLabel(item.action)}</Badge></TableCell><TableCell>{item.entityTitle ?? 'Không có tiêu đề'}</TableCell></TableRow>)}</tbody></DataTable> : <EmptyState title="Chưa có hoạt động" description="Các thay đổi quan trọng sẽ xuất hiện tại đây." />; }

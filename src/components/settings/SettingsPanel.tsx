...
export function SettingsPanel({ connections }: { connections: { firebase: boolean; graph: boolean; ai: boolean } }) {
  ...
  return <div className="grid gap-5">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      ...
      <Card><div className="flex items-center gap-3"><div className="rounded-xl bg-gold/18 p-3 text-bronze"><UserRound className="h-5 w-5" /></div><div><CardTitle>Hồ sơ quản trị</CardTitle><p className="text-sm text-stone-500">Đồng bộ Firebase Auth và admin_users.</p></div></div>
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      ...
      <Card><div className="flex items-center gap-3"><div className="rounded-xl bg-gold/18 p-3 text-bronze"><Database className="h-5 w-5" /></div><div><CardTitle>Kết nối hệ thống</CardTitle><p className="text-sm text-stone-500">Chỉ hiển thị trạng thái, không lộ khóa bí mật.</p></div></div><div className="mt-5 grid gap-3"><Connection icon={Database} label="Firebase Admin" connected={connections.firebase} /><Connection icon={Workflow} label="Neo4j Graph" connected={connections.graph} /><Connection icon={Zap} label="AI Provider" connected={connections.ai} /></div><p className="mt-4 text-xs leading-5 text-stone-500">Các biến môi trường được quản lý trên server. Sau khi thay đổi `.env`, cần khởi động lại Web Admin.</p></Card>
    </div>

    ...
    <form onSubmit={submit} className="grid gap-5">
      <Card>
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          ...
          <Field label="Slug" hint="Chữ thường, số và dấu gạch ngang; không đổi slug sau khi tạo.">
          ...
          <Field label="Ảnh bìa / coverMediaRef"><Input value={String(values.coverMediaRef ?? '')} onChange={(e) => set('coverMediaRef', e.target.value)} /></Field>
          <Field label="Tags" hint="Mỗi dòng một tag"><Textarea value={lines('tags')} onChange={(e) => set('tags', e.target.value)} /></Field>
        </div>
      </Card>
      ...
      {kind === 'stage' ? (
        <Card><CardTitle>Nội dung giai đoạn</CardTitle><div className="mt-4 grid gap-4 lg:grid-cols-2"><Field label="Overview"><Textarea value={String(values.overview ?? '')} onChange={(e) => set('overview', e.target.value)} /></Field><Field label="Description"><Textarea value={String(values.description ?? '')} onChange={(e) => set('description', e.target.value)} /></Field><Field label="Chi tiết (mỗi dòng một ý)"><Textarea value={lines('details')} onChange={(e) => set('details', e.target.value)} /></Field><Field label="Kết quả"><Textarea value={lines('result')} onChange={(e) => set('result', e.target.value)} /></Field><Field label="Tác động hiện tại"><Textarea value={String(values.impactOnPresent ?? '')} onChange={(e) => set('impactOnPresent', e.target.value)} /></Field><Field label="Nhân vật liên quan (slug)"><Textarea value={lines('relatedPersons')} onChange={(e) => set('relatedPersons', e.target.value)} /></Field></div></Card>
      ) : null}
      ...
      {kind === 'event' ? (
        <>
          ...
          <Card><CardTitle>Media và quan hệ</CardTitle><div className="mt-4 grid gap-4 lg:grid-cols-2"><Field label="Danh sách ảnh" hint="Mỗi dòng một URL"><Textarea value={lines('imageUrls')} onChange={(e) => set('imageUrls', e.target.value)} /></Field><Field label="Danh sách video" hint="Mỗi dòng một URL"><Textarea value={lines('videoUrls')} onChange={(e) => set('videoUrls', e.target.value)} /></Field><Field label="YouTube ID"><Input value={String(values.youtubeId ?? '')} onChange={(e) => set('youtubeId', e.target.value)} /></Field><Field label="Nhân vật liên quan"><Textarea value={lines('relatedPersons')} onChange={(e) => set('relatedPersons', e.target.value)} /></Field><Field label="Sự kiện liên quan"><Textarea value={lines('relatedEvents')} onChange={(e) => set('relatedEvents', e.target.value)} /></Field><Field label="Địa điểm liên quan"><Textarea value={lines('relatedLocations')} onChange={(e) => set('relatedLocations', e.target.value)} /></Field></div></Card>
        </>
      ) : null}
